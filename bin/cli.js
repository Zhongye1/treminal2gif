    ...
    try {
        ...
        if (options.exec) {
            const commands = options.exec.includes('&&')
                ? options.exec.split('&&').map((c) => c.trim())
                : [options.exec];
...
program
    ...
    try {
        ...
        if (!isCanvasAvailable()) {
            console.error(chalk.red('错误: Canvas 模块未正确安装，无法进行渲染'));
            console.log(chalk.yellow('\n在 Windows 上需要安装 GTK 库:'));
            console.log(chalk.gray('1. 下载 GTK 安装程序:'));
            console.log(chalk.gray('   https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer'));
            console.log(chalk.gray('2. 安装 GTK 后运行: npm rebuild canvas'));
            console.log(chalk.gray('3. 重新执行渲染命令'));
            process.exit(1);
        }
        const renderOptions = {};
        if (options.fps)
            renderOptions.frameRate = options.fps;
        if (options.quality)
            renderOptions.recording = { quality: options.quality };
        ...
        console.error(chalk.red('渲染失败:'), error.message);
        process.exit(1);
    }
});
// list 命令
